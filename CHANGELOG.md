...
- **`tf ralph start --interactive-session` no-op loops** - interactive backend now executes deterministic IRF workflow (`tf irf`) in Pi shell mode instead of relying on `/tf` prompt-template execution.
  - Prevents repeated iterations where `/tf` only prints `/chain-prompts ...` and exits 0
  - Keeps interactive-session behavior (`/exit` after command) while ensuring actual workflow execution

## [1.1.8] - 2026-02-18

### Fixed

- **`tf ralph start` queue-state crash** - Prevented `ValueError: Tickets cannot be both running and completed` during serial loop bookkeeping.
  - Queue snapshot builder now self-heals overlapping ticket sets (`running`/`completed`/`pending`) instead of crashing
  - Keeps Ralph loop alive even when a ticket reappears in `tk ready` after a no-op workflow run

## [1.1.7] - 2026-02-18

### Fixed

- **`/tf` prompt non-execution** - Fixed cases where `/tf <ticket>` printed a `/chain-prompts ...` command with explanatory text but did not execute.
  - Updated `/tf` prompt + `tf-workflow` skill with a strict output contract
  - For normal runs, response must be exactly one `/chain-prompts ...` line (no prose/tables/code fences)
  - Keeps `--plan` / `--dry-run` behavior as planning-only output

## [1.1.6] - 2026-02-18

### Fixed

- **`tf ralph start` prompt detection in worktrees** - Prevented false `Missing /tf prompt` failures when running tickets from per-ticket worktrees.
  - Ralph now ensures `/tf` prompt files are available in worktrees before launching Pi
  - If prompt assets exist in the launch project root but are not committed, they are copied into the worktree as a compatibility step
  - Keeps existing behavior for global prompt fallback and still fails with a clear error when no prompt exists at all

## [1.1.5] - 2026-02-18

### Fixed

- **`tf ralph` startup crash in uvx installs** - Fixed `FileNotFoundError` for `tf/ralph/session_recovery.py` when running commands like `tf ralph start --help`.
  - `tf.session_recovery` no longer depends on loading a missing file from `tf/ralph/` at runtime
  - Works in packaged layouts where `tf/ralph/` is not present
...
- **`tf ralph` import crash** - Fixed `ModuleNotFoundError: No module named 'tf.ralph.queue_state'; 'tf.ralph' is not a package` when running commands like `tf ralph start --help` from packaged installs where `tf.ralph` resolves to module form.
